(function (root) {
  ...
  function buildWorkbook(options) {
    ...
    hotels.forEach((hotel, hotelIndex) => {
      ...
      setCell(sheet, row, 0, hotel.name || "Hotel", hotelStyle);
      ...
      const suppliedRooms = Array.isArray(hotel.rooms) ? hotel.rooms : [];
      ...
      regularRooms.concat(allRoomsEntry).forEach(roomData => {
        ...
        const hasSubject = [...subjectSet].some(day => day >= 1 && day <= daysInMonth && !stopSet.has(day));
        ...
        const availableDays = [...subjectSet].filter(day => day >= 1 && day <= daysInMonth && !stopSet.has(day));
    ...
    sheet["!ref"] = XLSX.utils.encode_range({ s: { r: 0, c: 0 }, e: { r: row + 3, c: lastCol } });
    sheet["!merges"] = merges;
    sheet["!cols"] = [{ wch: 54 }].concat(Array.from({ length: daysInMonth }, () => ({ wch: 9 })));
    sheet["!rows"] = rows;
    ...
    const sheetName = `${monthNames[month]} ${year}`.slice(0, 31);
    const workbook = XLSX.utils.book_new();
    workbook.Props = {
      Title: `Stop Sale Calendar - ${monthNames[month]} ${year}`,
